...
func preRun(configPath string) error {
	...
	if runtime.GOOS == "windows" {
		...
		if arch != hostArch {
			//return fmt.Errorf("与当前系统不匹配，当前运行 %s_%s, 需要下载 %s_%s", runtime.GOOS, arch, runtime.GOOS, hostArch) //diy 修复Win下的错误
		}
	}
...
var probeHTML embed.FS //diy 把静态文件编译进二进制文件
func run() {
	...
	go func() {
		...
		http.HandleFunc("/basic", func(w http.ResponseWriter, r *http.Request) {
			...
			if err := json.Unmarshal(hostBytes, &hostmap); err != nil {
				log.Fatalf("Failed to unmarshal host: %v", err)
			}
			...
			mergedBytes, err := json.Marshal(merged) // 序列化合并后的 map
			if err != nil {
				log.Fatalf("Failed to marshal merged map: %v", err)
			}
			w.Write(mergedBytes)
		})
		// 提供静态文件服务(HTML页面)
		...
		http.HandleFunc("/garbage", func (w http.ResponseWriter, r *http.Request) {
			...
			if ckSize != "" {
				...
				if err != nil {
					printf("Invalid chunk size: %s", ckSize)
					printf("Will use default value %d", chunks)
				} else {
			...
			for i := 0; i < chunks; i++ {
				// 生成随机数据
				chunkSize := 1 * 1024 * 1024 //单个测试数据大小 (1MB)
				data := make([]byte, chunkSize)
				if _, err := rand.Read(data); err != nil {
					printf("Failed to generate random data: %s", err)
					return
				}
				if _, err := w.Write(data); err != nil {
					printf("Error writing back to client at chunk number %d: %s", i, err)
					break
				}
			}
		})
		port := strconv.Itoa(int(agentConfig.ProbeTestPort))
		address := fmt.Sprintf(":%s", port)
		if err := http.ListenAndServe(address, nil); err != nil {
			printf("Speed test server starting on %s: %v", address, err)
		}
	}()
	//diy
